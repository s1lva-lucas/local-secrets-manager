#!/usr/bin/env python3
"""
Wrapper for secrets_manager.py to use in other Python applications.
"""
...
class CredentialManagerWrapper:
    """Wrapper class for secrets_manager.py script."""
    ...
    def __init__(self, script_path: Optional[str] = None, prefix: str = "mcp", use_uv: bool = True):
        ...
        Args:
            script_path: Path to secrets_manager.py (auto-detects if None)
        ...
        if script_path:
            ...
            self.script_path = Path(__file__).parent / "secrets_manager.py"
        
        if not self.script_path.exists():
            raise FileNotFoundError(f"secrets_manager.py not found at {self.script_path}")
